...
from the gitmoji catalogue; `:t-rex:` is the one shortcode a delimited
slot cannot hold, so only its `🦖` is kept. An emoji written without its
variation selector (`⚡` for `⚡️`) is not matched.
