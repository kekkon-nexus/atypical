...
emoji or a `:shortcode:` intention. They cannot be referenced remotely
yet: copy one in, or vendor the repository.
...
extending file last. A `./` or absolute path is a file relative to the
extending file; a `npm:` specifier resolves through `node_modules`
(npm, pnpm, yarn or bun); a bare name is a neighbouring file when one
exists, else an npm package. Tables merge by key; `[[commit.slots]]`
merges by `name`:
